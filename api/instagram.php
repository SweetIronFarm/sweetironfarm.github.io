...
if(!isset($_GET['api'])) {
    ...
    if(isset($_GET['url'])) {
        $url = 'https://sparkleapp.com/api/instagram_oembed.php?url=' . $_GET['url'] . '&uuid=64332be858cfa7.62650570';
    ...
    if(function_exists('apcu_enabled') && apcu_enabled()) {
        $apcu = true;
        $access_token = apcu_fetch('ig_64332be858cfa7.62650570');
    }
    if($access_token === NULL || strlen($access_token) == 0) {
        $url = 'https://sparkleapp.com/api/instagram_token.php?uuid=64332be858cfa7.62650570';
        $access_token = url_get_contents($url);
        if($apcu && $access_token && strlen($access_token) > 0) {
            apcu_store('ig_64332be858cfa7.62650570', $access_token, 86400);
        }
    }
    if($access_token && strlen($access_token) > 0) {
        ...
        if($res === false) {
            ...
            apcu_store('ig_recent_64332be858cfa7.62650570', $res, 60);
        }
    }
    else {
        $err = 'invalid token uuid: 64332be858cfa7.62650570';
    }
}
else {
